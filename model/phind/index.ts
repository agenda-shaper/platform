...
let turndownService = new TurndownService({ codeBlockStyle: "fenced" }).remove(
  "h6"
);
...
export class Phind extends Chat implements BrowserUser<Account> {
  ...
  public static async copyContent(page: Page) {
    await page.waitForSelector(
      ".row > .col-lg-8 > .container-xl > .mb-4 > .btn:nth-child(3)",
      { timeout: 5 * 60 * 1000 }
    );
    await page.click(
      ".row > .col-lg-8 > .container-xl > .mb-4 > .btn:nth-child(3)"
  ...
  public async askStream(req: ChatRequest, stream: EventStream) {
    ...
    try {
      await page.waitForSelector(
        ".col-md-10 > .container-xl > .mb-3 > .input-group > .form-control"
      );
      await page.click(
        ".col-md-10 > .container-xl > .mb-3 > .input-group > .form-control"
      );
      ...
      await page.waitForSelector(
        ".col-lg-10 > .row > .col-lg-8:nth-child(4) > .container-xl > div:nth-child(1)"
      );
      ...
      const wait = async () => {
        ...
        const text: any =
          //@ts-ignore
          (await page.evaluate(() => navigator.clipboard.text)) || "";
        const sourcehtml: any = await output?.evaluate((el: any) => {
          return el.outerHTML;
        });
        console.log("chat end: ", text);
        const sourceText = turndownService
          .turndown(sourcehtml)
          .replace(preText, "");
